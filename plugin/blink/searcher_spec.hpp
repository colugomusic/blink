...
struct RealSearcherSpec
{
	std::function<float(const blink_RealPoints* points, float default_value, blink_Position block_pos, int search_beg_index, int* left)> binary;
	std::function<float(const blink_RealPoints* points, float default_value, blink_Position block_pos, int search_beg_index, int* left)> forward;
};
