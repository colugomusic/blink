...
inline float envelope_search(const blink_EnvelopePoints* points, blink_Position block_position, int* search_index)
{
	...
	if (*search_index < 0)
	{
		const auto find = [](blink_Position a, blink_EnvelopePoint point)
		{
